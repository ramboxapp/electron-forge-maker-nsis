...
export default class MakerNSIS extends MakerBase {
    ...
    async createAppUpdateYml(options, outPath) {
        ...
        if (provider === 'github') {
            ...
            const updaterCacheDirName = this.config.updater.updaterCacheDirName || `${options.appName.toLowerCase()}-updater`;
...
updaterCacheDirName: ${updaterCacheDirName}`;
            ...
            ymlContents = await getAppUpdateYml({
                ...
                publisherName: typeof this.config.updater.publisherName === 'string'
                    ? this.config.updater.publisherName
                    : undefined,
            });
        }
